...
// --- VISIBILITY RECOVERY ---
// Wenn der Tab pausiert war und wieder aktiv wird, verliert Canvas seinen Kontext.
// resize() zwingt ECharts, den gesamten Canvas neu zu zeichnen.
const handleVisibilityChange = () => {
  if (document.visibilityState !== 'visible') return;

  // Kleines Delay, damit der Browser den Rendering-Kontext vollständig wiederhergestellt hat
...
export const initChartWidget = () => {
  ...
  // Visibility-Listener registrieren
  ...
  elements.forEach(el => {
    ...
    if (isVisible) {
      render(el);
      // ResizeObserver erst nach dem async render einhängen
      setTimeout(() => { if (resizeObserver) resizeObserver.observe(el); }, 150);
    } else {
  ...
  resizeObserver = new ResizeObserver(entries => {
    ...
    resizeRaf = requestAnimationFrame(() => {
      entries.forEach(e => {
        ...
        if (item?.chart && !item.chart.isDisposed()) item.chart.resize();
      });
    });
  });
  ...
  mutationObserver = new MutationObserver(mutations => {
    if (mutations.some(m => m.attributeName === 'class')) {
      ...
      chartStore.forEach(({ chart }) => {
        if (!chart.isDisposed()) chart.setOption(themeConfig);
      });
    }
  });
